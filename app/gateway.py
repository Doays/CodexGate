...
from .policy import Decision, PolicyError, budget_for, collect_git_paths, validate_decision_for_run, validate_project_file, validate_selection, validate_workspace
...
class Approval:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "request_id": self.request_id, "method": self.method, "kind": self.kind,
            "thread_id": self.thread_id, "turn_id": self.turn_id, "item_id": self.item_id,
            "command": self.command, "cwd": self.cwd, "paths": self.paths, "reason": self.reason,
            "permissions": self.permissions, "available_decisions": list(self.available_decisions), "status": self.status,
        }
...
class Gate:
    ...
    def status(self) -> dict[str, Any]:
        return {
            "connected": self.client.connected, "models": self.client.models, "stderr": self.client.stderr_lines[-5:],
            "codex_path": self.client.command, "codex_version": self.client.version,
            "schema_path": str(self.client.schema_dir) if self.client.protocol else None,
            "schema_error": self.client.schema_error, "workspace_write_available": False,
            "workspace_write_schema_ready": self.client.workspace_write_schema_ready,
        }

    async def start_run(self, payload: dict[str, Any]) -> Run:
        models = await self.connect()
        if payload["permission"] == "workspace-write":
            raise PolicyError("Workspace Write는 실제 쓰기 검증 전까지 정책상 잠겨 있습니다.")
        ...
        run = Run(
            id=str(uuid.uuid4()), project_id=payload["project_id"], root=str(root), task=payload["task"].strip(),
            model=payload["model"], effort=payload["effort"], permission=payload["permission"],
            budget_level=payload["budget_level"], budget=budget, decision=decision.as_dict(),
        )
        ...
        self.store.save_task(run.id, run.project_id, run.status, run.snapshot())
        sandbox = "read-only" if run.permission == "read-only" else "workspace-write"
        thread = await self.client.request("thread/start", {
            "cwd": run.root, "sandbox": sandbox, "approvalPolicy": "on-request", "model": run.model,
            "threadSource": "codex_gate", "ephemeral": False, "approvalsReviewer": "user",
        })
        run.thread_id = thread["thread"]["id"]
        turn = await self.client.request("turn/start", {
            "threadId": run.thread_id, "model": run.model, "effort": run.effort,
            "cwd": run.root, "approvalPolicy": "on-request", "approvalsReviewer": "user",
            "sandboxPolicy": self._sandbox_policy(run),
            "input": [{"type": "text", "text": self._prompt(run)}],
        })
        run.turn_id = turn["turn"]["id"]
        run.status = "running"
        await self._publish(run, "새 Codex 스레드에서 작업을 시작했습니다.")
        self._persist(run)
        return run
    ...
    def _prompt(self, run: Run) -> str:
        ...
        return "\n".join([
            "# Codex Gate 실행 지시", "이 작업은 독립된 새 스레드입니다. 아래 범위와 중단 조건을 엄격히 지키세요.",
            f"작업: {run.task}", f"권한: {run.permission}", f"허용 파일: {json.dumps(decision['allowed_files'], ensure_ascii=False)}",
            f"금지 파일: {json.dumps(decision['forbidden_files'], ensure_ascii=False)}",
            f"검증 명령: {json.dumps(decision['validation_commands'], ensure_ascii=False)}",
            f"중단 조건: {json.dumps(decision['stop_conditions'], ensure_ascii=False)}",
            "허용 범위 밖 파일, DB migration, 운영 변경, 새 설계 판단이 필요하면 즉시 멈추고 이유만 보고하세요. "
            "Ultra 및 서브에이전트 생성은 금지됩니다. 파일을 바꾸기 전에는 해당 파일이 허용 목록에 있는지 확인하세요.",
        ])

    async def interrupt(self, run_id: str, reason: str = "사용자 중단") -> None:
        run = self._run(run_id)
        if run.status not in {"running", "starting"} or not run.thread_id or not run.turn_id:
            return
    ...
    async def handle_event(self, message: dict[str, Any]) -> None:
        ...
        run = next((item for item in self.runs.values() if item.thread_id == thread_id), None)
        if not run:
            return
        method = message["method"]
        if method == "serverRequest/resolved":
            self._resolve_server_request(run, params)
            await self._publish(run, "server request 해결됨")
            self._persist(run)
            return
        if method == "thread/tokenUsage/updated":
            ...
            await self._handle_item(run, item, count_tool=method == "item/started")
            ...
            await self._publish(run, f"턴 완료: {run.status}")
    ...
    async def handle_server_request(self, message: dict[str, Any]) -> None:
        ...
        if not isinstance(available, list) or not all(isinstance(item, str) for item in available):
            available = list(self.client.protocol.decisions_for(kind) if self.client.protocol else ())
        paths = []
        ...
        approval = Approval(
            request_id=message["id"], method=method, kind=kind, thread_id=thread_id, turn_id=params.get("turnId", ""),
            item_id=params.get("itemId"), command=params.get("command"), cwd=params.get("cwd"), paths=list(dict.fromkeys(paths)),
            reason=params.get("reason"), permissions=params.get("permissions"), available_decisions=tuple(available),
        )
        run.approvals[key] = approval
        approval.timeout_task = asyncio.create_task(self._expire_approval(run.id, approval.key))
        await self._publish(run, f"사용자 승인 대기: {kind}")
        self._persist(run)

    async def resolve_approval(
        self, run_id: str, request_id: int | str, decision: str, granted_permissions: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        run = self._run(run_id)
        ...
        async with approval.lock:
            ...
            await self._publish(run, f"승인 응답: {approval.kind} / {decision}")
            self._persist(run)
    ...
    async def _handle_item(self, run: Run, item: dict[str, Any], count_tool: bool) -> None:
        ...
        if item_type in {"collabToolCall", "collabAgentToolCall"} and str(item.get("tool", "")).replace("_", "").lower() == "spawnagent":
            await self.interrupt(run.id, "서브에이전트 생성 감지")
            return
    ...
    async def _handle_changes(self, run: Run, changes: list[dict[str, Any]]) -> None:
        for change in changes:
            ...
            if not path:
                continue
            run.changed_files.add(path.replace("\\", "/"))
            if not self._is_allowed(run, path):
                await self.interrupt(run.id, f"허용 파일 외 수정 감지: {path}")
                return
        await self._enforce(run)
    ...
    async def _enforce(self, run: Run) -> None:
        if run.tokens >= run.budget["tokens"]:
            ...
            await self.interrupt(run.id, "같은 유형의 명령 실패 한도 도달")
    ...
    async def _capture_diff(self, run: Run) -> None:
        try:
            completed = await asyncio.to_thread(
                subprocess.run, ["git", "diff", "--no-ext-diff"], cwd=run.root,
    ...
    async def _recheck_git_paths(self, run: Run) -> None:
        for path in self._git_paths(run.root) - run.initial_git_paths:
            if not self._is_allowed(run, path):
                run.stop_reason = f"종료 후 Git 재검사에서 범위 밖 변경 감지: {path}"
                run.status = "failed"
                return

    async def _respond_unmatched_request(self, message: dict[str, Any]) -> None:
        kind = APPROVAL_METHODS[message["method"]]
        result = {"permissions": {}} if kind == "permissions" else {"decision": "cancel"}
        await self.client.respond(message["id"], result)
