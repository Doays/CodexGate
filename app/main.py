...
class CodexProcessCanaryRequest(BaseModel):
    ...
    permit_nonce: str | None = Field(default=None, min_length=32, max_length=128)


class CodexProcessCanaryArmRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
def codex_canary_execution_permit_gate(request: Request) -> CodexCanaryExecutionPermitGate:
    return request.app.state.codex_canary_execution_permit_gate
...
def codex_process_canary_snapshot(request: Request) -> dict[str, Any]:
    ...
    if readiness.get("status") == "READY":
        ...
        result = readiness
    permit_gate = codex_canary_execution_permit_gate(request)
    ...
    result["execution_enabled"] = (
        ...
        and result["execution_window"].get("status") == "ARMED"
    )
    return result
...
@app.post("/api/isolation/wsl/codex-process-canary/permit")
async def issue_codex_process_canary_permit(request: Request):
    rejection = _enforce_execution_window_arm_request(request)
    if rejection is not None:
        return rejection
    try:
